  ...
  <main class="container">
    <Sidebar>
      ...
      <div class="sidebar-section" v-if="!editing">
        <SelectInput
          label="Resume format"
          :options="[{'name': 'DIN A4', 'value': 'a4'}, {'name': 'Letter', 'value': 'letter'}]"
      ...
      <div class="sidebar-section" v-if="!editing">
        <small style="display:block;margin-bottom:10px;">
          A file with your configuration will be downloaded to your computer. You can upload it whenever you want to restore your settings.
        </small>
    ...
    <div class="resume-wrapper">
      ...
      <div id="resume"
        ...
        <div class="left-col" :style="{width: percentageWidthLeft}">
          ...
          <ResumeSection>
            <SectionHeadline :headline="headlines[0]" :editing="editing" @headline-edited="updateHeadline($event, 0)" />
            <p :contenteditable="editing" @input="updateProperty($event, 'introText')">{{ introText }}</p>
          </ResumeSection>

          <ResumeSection>
            <SectionHeadline :headline="headlines[1]" :editing="editing" @headline-edited="updateHeadline($event, 1)" />
            <Contact
              :contact="contact"
              :editing="editing"
          ...
          <ResumeSection>
            <SectionHeadline :headline="headlines[2]" :editing="editing" @headline-edited="updateHeadline($event, 2)" />
            <ul>
          ...
          <ResumeSection>
            <SectionHeadline :headline="headlines[3]" :editing="editing" @headline-edited="updateHeadline($event, 3)" />
            <ul>
              <li v-for="(highlight, index) in highlights" :key="index" :contenteditable="editing"
                @input="updateNestedProperty($event, 'highlights', index)">{{ highlight }}</li>
            </ul>
        ...
        <div class="right-col">
          <div class="personal-name" :contenteditable="editing" @input="updateProperty($event, 'name')">{{ name }}</div>
          <h3 class="personal-title" :contenteditable="editing" @input="updateProperty($event, 'title')">{{ title }}</h3>

          <ResumeSection>
            <div class="d-flex">
              <SectionHeadline :headline="headlines[4]" :editing="editing" @headline-edited="updateHeadline($event, 4)" />
              <EditButtons @add-click="addExperience" :show-remove-btn="false" text-add="Add experience" />
            </div>

            <div v-for="(item, index) in experience" :key="index">
              <EditButtons @remove-click="removeExperience(index)" :show-add-btn="false" btn-align="right" move-down="true"/>
              <ResumeEntry
                :item="item"
                :editing="editing"
                @edit="($event, key) => updateExperience($event, key, index)"
                @edit-desc="($event, innerIndex) => updateExperienceDescription($event, index, innerIndex)"
              />

              <EditButtons @add-click="addDescriptionItem('experience', index)"
          ...
          <ResumeSection>
            <div class="d-flex">
              <SectionHeadline :headline="headlines[5]" :editing="editing" @headline-edited="updateHeadline($event, 5)" />
              <EditButtons @add-click="addEducation" :show-remove-btn="false" text-add="Add education" />
            </div>

            <div v-for="(item, index) in education" :key="index">
              <EditButtons @remove-click="removeEducation(index)" :show-add-btn="false" btn-align="right" move-down="true"/>
              <ResumeEntry
                :item="item"
                :editing="editing"
                @edit="($event, key) => updateEducation($event, key, index)"
                @edit-desc="($event, innerIndex) => updateEducationDescription($event, index, innerIndex)"
              />

              <EditButtons @add-click="addDescriptionItem('education', index)"
